...
RUN apt install --no-cache coreutils sqlite-dev libxml2-dev curl-dev gmp-dev icu-dev libpng-dev jpeg-dev freetype-dev autoconf imagemagick-dev gcc libc-dev libzip-dev rabbitmq-c-dev make libtool \
...
RUN chmod +x /usr/local/bin/confd \
	&& apt install --no-cache sqlite libxml2 curl gmp icu libpng jpeg freetype libzip imagemagick gcc ssmtp rabbitmq-c \
	# Fix alpine iconv problems part 1
	# See https://github.com/docker-library/php/issues/240
	&& apt install --no-cache --repository http://dl-cdn.alpinelinux.org/alpine/edge/community gnu-libiconv \
...
RUN apt install --update supervisor && rm -rf /tmp/* /var/cache/apk/*
